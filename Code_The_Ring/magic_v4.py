...
# cover consequent duplicate case, consider to reserve more frequent letter
...
def most_frequent(spell_num):
    ocr = [(x, 0) for x in range(27)]
    for i in spell_num:
        ocr[i] = (ocr[i][0], ocr[i][1]+1)
    ocr.sort(key=lambda tup: tup[1], reverse=True)
    return ocr
...
def find_best_zone(zone, cur, target, ocr_list):
    best, move_index = 500, cur
    find = False
    reserve = [False for i in range(len(ocr_list))]
    for j in range(30):
        # test if it's reserved
        skip = False
        for o in range(len(ocr_list)):
            if ocr_list[o][0] == zone[j] and ocr_list[o][0] != target:
                skip = True
        if skip and reserve[o] is False:
            reserve[o] = True
            continue
...
# change consequent left zones to cur to c_target
def gen_multi(zone, cur, c_target, dup_times, ocr_list):
    tzone = zone[:]
    total = c_target * dup_times
    tcur = cur
    instr = ''
    for i in range(len(total)):
        tcur, seq = find_best_zone(tzone, tcur, total[i], ocr_list)
        instr += seq + '.'
        tzone[tcur] = total[i]
...
def greedy(spell):
    ...
    ocr_list = most_frequent(spell_num)
    ocr_list = ocr_list[:1]
    i = 0
    while i < len(spell_num):
        if i in dup_table:
            ...
            while iter_dup > 0:
                ...
                # not use loop
                nch_zone, nseq, ntzone = gen_multi(zone, cur_zone, dup_ch, iter_dup, ocr_list)
                if len(seq) < len(nseq):
                    cur_zone = t_zone
                    zone = tzone
                    instr += seq
            ...
            # normal case
            cur_zone, seq, zone = gen_multi(zone, cur_zone, [spell_num[i]], 1, ocr_list)
            instr += seq
            i += 1
    return instr
